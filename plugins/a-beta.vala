...
class AXIOME.Analyses.BetaDiversity : RuleProcessor {
	...
	public override bool process(Xml.Node *definition, Output output) {
		...
		if (pipeline.to_string() == "mothur") {
			//MOTHUR Beta:
			definition_error(definition, "Beta plugin not available for mothur yet. Sorry! Skipping...\n");
			...
			output.add_rule(@"ifeq ($$(QIIME_GREATER_THAN_1_5),TRUE)\nBETA_SCALE_FLAG:=-s scaled,unscaled\nendif\nbiplot_coords_$(taxname)$(flavour).txt biplot_coords_$(taxname)$(flavour)_unweighted.txt: beta_div_pcoa$(flavour)/pcoa_weighted_unifrac_otu_table$(flavour).txt prefs_$(taxname)$(flavour).txt otu_table_summarized_$(taxname)$(flavour).txt\n\t@echo Producing biplot 3D image $(flavour) at $(taxname)-level...\n\t$$(V)test ! -d biplot$(taxname)$(flavour) || rm -rf biplot$(taxname)$(flavour)\n\t$$(V)$$(QIIME_PREFIX)make_3d_plots.py $$(BETA_SCALE_FLAG) -t otu_table_summarized_$(taxname)$(flavour).txt -i beta_div_pcoa$(flavour)/pcoa_weighted_unifrac_otu_table$(flavour).txt -m mapping.txt -p prefs_$(taxname)$(flavour).txt -o biplot$(taxname)$(flavour) --biplot_output_file biplot_coords_$(taxname)$(flavour).txt --n_taxa_keep=$(taxakeep)\n\t$$(V)$$(QIIME_PREFIX)make_3d_plots.py $$(BETA_SCALE_FLAG) -t otu_table_summarized_$(taxname)$(flavour).txt -i beta_div_pcoa$(flavour)/pcoa_unweighted_unifrac_otu_table$(flavour).txt -m mapping.txt -p prefs_$(taxname)$(flavour).txt -o biplot$(taxname)$(flavour)_unweighted --biplot_output_file biplot_coords_$(taxname)$(flavour)_unweighted.txt --n_taxa_keep=$(taxakeep)\n\n");
